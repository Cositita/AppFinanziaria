...
def parse_hype_notification(text):
  if not text:
    return None

  # Filtro di sicurezza rigido: se c'è qualsiasi termine di rifiuto/storno, scarta subito
  parole_chiave_scarto = [
      "rifiutato",
      "rifiutata",
      "negato",
      "negata",
  ...
  if any(parola in text.lower() for parola in parole_chiave_scarto):
    return None

  # Cerca l'importo nel formato reale visto nelle notifiche Hype (es. "33,17 €" o "euro")
  amount_match = re.search(r"(\d+[\.,]?\d*)\s*€", text)
  if not amount_match:
    amount_match = re.search(r"(\d+[\.,]?\d*)\s*euro", text, re.IGNORECASE)

  if amount_match:
    raw_importo = amount_match.group(1).replace(",", ".")
    try:
      importo = float(raw_importo)
    except ValueError:
      return None

    # Estrae l'esercente pulendo il testo
    esercente = text.replace(amount_match.group(0), "").strip(" ,\n")
...
def main(page: ft.Page):
  ...
  anni_disponibili = [
      str(y) for y in range(anno_corrente_int - 5, anno_corrente_int + 6)
  ]
  ...
  dd_filtro_anno = ft.Dropdown(
      ...
      value=(
          anno_corrente_str
          if anno_corrente_str in anni_disponibili
          else anni_disponibili[0]
      ),
      on_select=cambia_anno_filtro,
      width=200,
  )

  lbl_saldo_bcc = ft.Text(
      "€ 0.00", size=20, weight=ft.FontWeight.BOLD, color="#66bb6a"
  )
  lbl_proiettato_bcc = ft.Text("€ 0.00", size=14, color="#bdbdbd")

  lbl_saldo_hype = ft.Text(
      "€ 0.00", size=20, weight=ft.FontWeight.BOLD, color="#66bb6a"
  )
  lbl_proiettato_hype = ft.Text("€ 0.00", size=14, color="#bdbdbd")

  lbl_saldo_totale = ft.Text(
      "€ 0.00", size=20, weight=ft.FontWeight.BOLD, color="#81d4fa"
  )
  ...
  container_avvisi = ft.Container(
      visible=False, padding=12, bgcolor="#3b1c1c", border_radius=8
  )
  ...
  txt_importo = ft.TextField(
      label="Importo (€)", keyboard_type=ft.KeyboardType.NUMBER, expand=True
  )
  txt_descrizione = ft.TextField(
      label="Descrizione (es. Rata Mutuo)", expand=True
  )
  txt_data = ft.TextField(
      label="Data Inizio (YYYY-MM-DD)",
      value=data_corrente_odierna.strftime("%Y-%m-%d"),
      expand=True,
  )
  txt_ripetizione = ft.TextField(
      label="Mesi programmati (es. 3 per Ott-Nov-Dic, o 12)",
      value="1",
      expand=True,
  )
  txt_nuova_cat = ft.TextField(
      label="Nuova categoria personalizzata", expand=True
  )
  ...
  def aggiungi_nuova_categoria(e):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO categorie_custom (tipo, nome) VALUES (?, ?)",
        (tipo_corrente, nuova),
    )
    conn.commit()
    conn.close()
  ...
  btn_aggiungi_cat = ft.Button(
      content=ft.Text("Aggiungi Categoria"), on_click=aggiungi_nuova_categoria
  )

  lbl_titolo_form = ft.Text(
      "Aggiungi / Modifica Movimento Programmato",
      size=18,
  ...
  def controlla_avvisi_globali():
    ...
    cursor.execute(
        ...
        """,
        (
            data_corrente_odierna.strftime("%Y-%m-%d"),
            (data_corrente_odierna + timedelta(days=7)).strftime("%Y-%m-%d"),
        ),
    )
    ...
    if scadenze:
      sezione_avvisi.controls.append(
          ft.Text(
              "⚠️ ATTENZIONE: Pagamenti in scadenza nei prossimi 7 giorni!",
              weight=ft.FontWeight.BOLD,
              color="#ff5252",
              size=14,
          )
      ...
      for s_id, s_conto, s_tipo, s_imp, s_cat, s_desc, s_data in scadenze:
        ...
        sezione_avvisi.controls.append(
            ft.Text(
                f"• [{s_data} - {testo_giorni}] {s_conto} |"
                f" {s_cat}{desc_str}: € {s_imp:.2f}",
                color="#ff8a80",
                size=13,
            )
        )
  ...
  def carica_dati():
    ...
    data_limite_settimana = (
        data_corrente_odierna + timedelta(days=7)
    ).strftime("%Y-%m-%d")
    ...
    for (
      ...
      if (
          tipo == "Uscita"
          and stringa_oggi <= data_str <= data_limite_settimana
      ):
        sfondo_riga = "#3d241c"
      ...
      def prepara_modifica(
        ...
        txt_data.value = (
            dt if dt else data_corrente_odierna.strftime("%Y-%m-%d")
        )
      ...
      btn_modifica = ft.TextButton(
          content=ft.Text(
              "Modifica", color="#ffca28", weight=ft.FontWeight.BOLD
          ),
              ...
              [
                  ft.Column(
                      [
                          ft.Text(
                              f"[{data_str}] {conto} -"
                              f" {categoria or 'Senza categoria'}",
                              weight=ft.FontWeight.BOLD,
                              size=14,
                          ),
                          ft.Text(
                              f"{descrizione or 'Nessuna descrizione'}",
                              size=12,
                              color="#bdbdbd",
                          ),
                      ],
  ...
  def salva_movimento(e):
    ...
    try:
      ...
      page.snack_bar = ft.SnackBar(
          ft.Text("Errore nel formato dell'importo o dei mesi!")
      )
      page.snack_bar.open = True
      page.update()
      return
    ...
    try:
      ...
      page.snack_bar = ft.SnackBar(
          ft.Text("Formato data errato! Usa YYYY-MM-DD (es. 2026-09-12)")
      )
      page.snack_bar.open = True
      page.update()
      return
  ...
  sezione_storico = ft.ExpansionTile(
      title=ft.Text(
          "Storico Movimenti dell'Anno Selezionato",
          size=18,
          weight=ft.FontWeight.BOLD,
      ),
      bgcolor="#1e2530",
      collapsed_bgcolor="#181f29",
      controls=[ft.Container(content=lista_storico, padding=10)],
  )

  page.add(
      ft.Text(
          "Gestione Spese & Tasse Multi-Conto",
          size=24,
          weight=ft.FontWeight.BOLD,
      ),
      container_avvisi,
